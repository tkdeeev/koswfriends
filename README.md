...
The default week shows only the user's timetable, with stacked initials for friends attending the same Sirius event. Friends' overlays start off. The Your timetable checkbox can hide the user's lessons and drafts to compare friends only. Own lessons use a solid colored border and fill; friends use an outline with dashed top/bottom edges and a solid colored left accent. Names remain in attendee avatars and details. The All friends checkbox toggles all overlays at once; individual controls remain in the filters above the calendar. Weekday columns have equal widths and fit on screen. The week shows up to three parallel lanes; crowded blocks reserve the last lane for a count of more lessons and prioritize the user’s own lessons. Click a day heading or the overflow count to expand that day across the timetable, then use Back to week or Escape to return. Expanded days show every lesson; very dense days can scroll internally. Personal events receive the same left-lane priority as imported own lessons, even when a friend’s event starts earlier. On mobile, a compact day picker opens an hourly grid with proportional durations and gaps. Each selected person has an avatar-headed lane on the same time axis; two people fit side by side, and extra people or simultaneous lessons scroll inside the grid. Identical lessons join across adjacent participants when no concurrent lesson blocks the span. Shared cards keep their content visible while scrolling; separated participants retain linked cards with attendee avatars. Solid side edges mark the outer start/end of the block; dashed side edges mark continuation into another segment or beyond the scroll viewport. The Day layout setting switches mobile and expanded desktop days between person lanes and the compact lesson layout. It remembers the selection; the full desktop week retains its lesson layout. Different lesson IDs, drafts and cancellations are never joined. Filters collapse behind an icon and bottom navigation uses labeled SVG icons. Empty weekend days are hidden independently. Lesson accents use Fittable's published lecture, tutorial, laboratory and exam palette. Avatars use the first and sixth username letters and a stable full-spectrum username color with at least 4.5:1 contrast for white initials. The separate Usermap photo endpoint documents only public employee photos, so student photos are not imported; see [profile-photos.md](docs/profile-photos.md). No additional school scope is requested. Dark mode follows the system preference until changed with the header toggle, then remembers the choice. Calendar colors mix against the active theme surface, including custom colors, and dialogs and forms use the same theme.
