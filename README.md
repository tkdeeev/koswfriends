...
Both test commands create and remove a dedicated embedded PostgreSQL 17 database on loopback port 55438. Run them sequentially. Fixtures are synthetic and never deploy to production. With npm versions that block lifecycle scripts, approve `@embedded-postgres/linux-x64` (or the matching platform package) and rebuild it first. Tests cover directional sharing, pending invitations, immediate revocation, blocking, deletion, OAuth state replay, token identity checks and concurrent rotation, complete imports, provider failures, overlapping/cancelled lessons, and Prague DST. Browser checks use separate synthetic accounts and cover mobile overflow and Czech/English/Ukrainian text.
...
The default week shows only the user's timetable, with stacked initials for friends attending the same Sirius event. Friends' overlays start off. The Your timetable toggle can hide the user's lessons and drafts to compare friends only. Own lessons use a solid colored border and fill; friends use an outline with dashed top/bottom edges and a solid colored left accent. Names remain in attendee avatars and details. The All friends icon toggles all overlays at once; the same toggle and individual people remain in the filters above the calendar. Compact toggle buttons keep the four primary filters on one mobile row. Weekday columns have equal widths and fit on screen. The week shows up to three parallel lanes; crowded blocks reserve the last lane for a count of more lessons and prioritize the user’s own lessons. Click a day heading or the overflow count to expand that day across the timetable, then use Back to week or Escape to return. Expanded days show every lesson; very dense days can scroll internally. Personal events receive the same left-lane priority as imported own lessons, even when a friend’s event starts earlier. On mobile, a compact day picker opens an hourly grid with proportional durations and gaps. Each selected person has an avatar-headed lane on the same time axis; two people fit side by side, and extra people or simultaneous lessons scroll inside the grid. Identical lessons join across adjacent participants when no concurrent lesson blocks the span. The clock stays outside the horizontal scroller with fixed-width, non-wrapping time labels. Shared cards keep their content visible while scrolling; separated participants retain cards with attendee avatars. Solid side edges mark the outer start/end of the block; dashed side edges mark continuation into another segment or beyond the scroll viewport. Native sticky edges use the same dash pattern and do not depend on JavaScript scroll updates. The joined By person / By lesson icon buttons and the Day layout setting switch mobile and expanded desktop days between person lanes and the compact lesson layout. It remembers the selection; the full desktop week retains its lesson layout. Different lesson IDs, drafts and cancellations are never joined. Filters collapse behind an icon and bottom navigation uses labeled SVG icons. Empty weekend days are hidden independently. Lesson accents use Fittable's published lecture, exercise, laboratory and exam palette. Avatars use the first letters of the first and last display-name words (one initial for a single word), falling back to the first and sixth username letters and a stable full-spectrum username color with at least 4.5:1 contrast for white initials. The separate Usermap photo endpoint documents only public employee photos, so student photos are not imported; see [profile-photos.md](docs/profile-photos.md). No additional school scope is requested. Dark mode follows the system preference until changed with the header toggle, then remembers the choice. Calendar colors mix against the active theme surface, including custom colors, and dialogs and forms use the same theme.
...
## Public launch and licensing

The semester planner is a preview available at `/?view=planner`; it is intentionally absent from navigation and the landing page.

Public [privacy information](https://kos.deeev.cz/privacy) and [terms](https://kos.deeev.cz/terms) are available in Czech, English and Ukrainian. Account settings provide a private JSON data export and immediate account deletion. The worker removes accounts after 365 days without authenticated activity. Background synchronization does not renew retention.
